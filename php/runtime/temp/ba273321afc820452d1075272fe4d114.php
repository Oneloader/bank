...
<div class="page">
    ...
    <div class="flexigrid">
        <div class="mDiv" style="padding-bottom: 10px;">
            ...
            <form action="<?php echo U('bank/branch'); ?>" id="search-form2" class="navbar-form form-inline" method="post">
                <div class="sDiv">
                    <div class="sDiv2">
                        <input type="text" size="30" name="key_word" class="qsbox" placeholder="支行名称">
    ...
    <script>
        function addBank(id){
            var url = "<?php echo U('Admin/Bank/addEditBranch'); ?>?id="+id;
            layer.open({
                type: 2,
                title: '添加/编辑银行支行',
                shadeClose: true,
        ...
        function del(id) {
            layer.confirm('你确认要删除该银行支行吗？', function ()
            {
                // 确定
                $.ajax({
                    url: "<?php echo U('Admin/Bank/branch_del'); ?>",
                    type: 'post',
                    data: {id: id},
                    success: function (v) {
                        layer.closeAll();
