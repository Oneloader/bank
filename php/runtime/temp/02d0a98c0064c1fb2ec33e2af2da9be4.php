...
<div class="page" style="margin-top: -50px;">
    ...
    <script>
        function viewGoods(id){
            ...
            var url = "<?php echo U('Admin/Index/select_goods'); ?>?aid="+aid+'&level_id='+id;
            layer.open({
                type: 2,
                title: '查看活动商品',
                shadeClose: false,
        ...
        function addBank(id){
            var aid = '<?php echo (isset($aid) && ($aid !== '')?$aid:0); ?>';
            var url = "<?php echo U('Admin/Bank/addEditChangeLevel'); ?>?id="+id+'&aid='+aid;
            layer.open({
                type: 2,
                title: '添加/编辑兑换档次',
                shadeClose: true,
        ...
        function del(id) {
            layer.confirm('你确认要删除该兑换档次吗？', function ()
            {
                // 确定
                $.ajax({
                    url: "<?php echo U('Admin/Bank/change_level_del'); ?>",
                    type: 'post',
                    data: {id: id},
                    success: function (v) {
                        layer.closeAll();
