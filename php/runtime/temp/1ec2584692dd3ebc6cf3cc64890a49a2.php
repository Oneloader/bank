<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:40:"./template/bank/index\order_details.html";i:1541761755;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<div style="padding: 0 0 20px 0;background-color:<?php echo $data['back_color']; ?>">
    ...
    <div class="button_dui">
        <button id="submit" data-status="0" class="btn_orange" style="width: 100%;">提交信息</button>
    </div>
</div>
...
//            var jsonStr = JSON.stringify($("#form1").serializeObject());  //json字符串
            ...
            var url = "<?php echo url('Bank/Index/order_details',['sid'=>$sid,'aid'=>$aid]); ?>";
            $('.input_error').html('');
//            if(customer != ''){
            $.ajax({
                async:false,
...
//                data:{'customer':customer,'ID_num':ID_num,'consignee':consignee,'phone':phone,'province':province,'city':city,'district':district,'address':address,'good_id':good_id,'type':type,'shop_num':shop_num,'must_name_id':must_name_id,'must_user_phone':must_user_phone,'user_phone':user_phone},
                data:arr,
                type:'post',
                dataType:'json',
                success:function(res){
                    ...
                    if(res.status != 1){
                        ...
                        $('.cont_btn').click(function(){
                            location.href="<?php echo url('Bank/Index/goodsList',['sid'=>$sid,'aid'=>$activity_id,'level_id'=>$level_id]); ?>";
                            $('.center_box').fadeOut(200);
                        });
                    }
//                    }
