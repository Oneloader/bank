...
class Cx extends Taglib
{
    ...
    protected $tags = [
        ...
        'K11APP'     => ['attr'=>'sql,key,item,result_name','close'=>1,'level'=>3], // K11APP sql 万能标签
        'adv'        => ['attr'=>'limit,order,where,item','close'=>1],        
    ];

    
	public function tagK11APP($tag, $content)
        {
            ...
            $t  =  !empty($tag['t']) ? $tag['t'] : K11APP_CACHE_TIME;// 缓存时间
    ...
    {
        $sid = intval(base64_decode($_GET['sid']));
     	$order = $tag['order']; //排序
        ...
        $str = '<?php ';
        $str .= '$pid ='.$pid.';$sid='.$sid.';';
        $str .= '$ad_position = M("ad_position")->cache(true,K11APP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");';
        $str .= '$result = M("ad")->where("pid=$pid and sid=$sid  and enabled = 1 and start_time < '.strtotime(date('Y-m-d H:00:00')).' and end_time > '.strtotime(date('Y-m-d H:00:00')).' ")->order("orderby desc")->cache(true,K11APP_CACHE_TIME)->limit("'.$limit.'")->select();';
        $str .= '
