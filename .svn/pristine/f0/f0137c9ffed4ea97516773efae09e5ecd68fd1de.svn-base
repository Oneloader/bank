...
class GoodsLogic extends Model
{
    ...
    public function goods_cat_list()
    {
        ...
        $where = ' `sid`='.$this->sid;
        $sql = "SELECT * FROM  __PREFIX__goods_category WHERE ".$where." ORDER BY parent_id , sort_order ASC";
    ...
    {  
        ...
        // 没有被 unset($old_goods单_attr[$tmp_key]); 掉是 说明 数据库中存在 表中没有提交过来则要删除操作
    ...
    { 
        $model = M('SpecItem');        
        $arr = $model->where(array('spec_id' => $spec_id,'sid'=>$this->sid))->order('id')->select();
     ...
     */
    public function getSpecInput($goods_id, $spec_arr,$edit=false)
    {
        ...
        $spec = M('Spec')->where(array('sid'=>$this->sid))->getField('id,name'); // 规格表
        $specItem = M('SpecItem')->where(array('sid'=>$this->sid))->getField('id,item,spec_id');//规格项
        $keySpecGoodsPrice = M('SpecGoodsPrice')->where('goods_id = '.$goods_id)->getField('key,key_name,store_count,sku');//规格项
        ...
        $price_items = model('Goods')->price_items;
        //获取价格
        $price = M('spec_goods_price_detail')->where(array('goods_id'=>$goods_id))->select();
        // 显示第二行开始
        foreach ($spec_arr2 as $k => $v)
        {
            ...
            if(!empty($goods_id) && !isset($keySpecGoodsPrice[$item_key][key]) && $edit){
                continue;
            }else{
                $str .= $str1;
            }
            ...
            if(!empty($price)){
                ...
                foreach($price as $va){
                    if($item_key == $va['key']){
                        $p_arr = json_decode($va['price'],true);
                        $p_arr = array_filter($p_arr,create_function('$v','return !empty($v);'));
                        $p_str = implode('+',$p_arr);
                        //$price_str .= '<div class="price">' .$price_items[$ke]. '：'. $va . '<input type="hidden" name="item['. $item_key .'][spec_pay_val]['. $ke .']" value="' .$va . '"></div>';
                        $price_str .= '<div class="price">' .$price_items[$va['price_type']]. '：'. $p_str . '</div>';
                    }
                }
                $keySpecGoodsPrice[$item_key][price] = empty($price_str) ? 'price：0' : $price_str;
            }else{
                $keySpecGoodsPrice[$item_key][price] =  'price：0';
            }

			$keySpecGoodsPrice[$item_key][store_count] ? false : $keySpecGoodsPrice[$item_key][store_count] = 0; //库存默认为0
            $keySpecGoodsPrice[$item_key][sku] = empty($keySpecGoodsPrice[$item_key][sku]) ? 'KGSKU_'.$goods_id.'_'.$item_key : $keySpecGoodsPrice[$item_key][sku];
            $str .="<td class='price_box' box='$item_key' id='price_$item_key'><div class='price $item_key' >{$keySpecGoodsPrice[$item_key][price]}</div></td>";
            $str .="<td><button type='button' class='btn btn-default' onclick='setPrice(\"$item_key\");'>设置</button></td>";
            $str .="<td><input type='text' class='table_input store_input' name='item[$item_key][store_count]' value='{$keySpecGoodsPrice[$item_key][store_count]}' onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d.]/g,\"\")' onblur='fresh_store();' /></td>";
            $str .="<td><input type='text' name='item[$item_key][sku]' value='{$keySpecGoodsPrice[$item_key][sku]}' /></td>";
            $str .="<input type='hidden' name='item[$item_key][key_name]' value='$item_name' />";
            $str .="</tr>";
        }
        if(!empty($keySpecGoodsPrice)){
            $str .= '<tr><td>批量设置</td><td colspan="4"><a href="javascript:void(0)" onclick="setAllPrice();">价格</a> &emsp;&emsp; <a href="javascript:void(0)" onclick="setAllStore();">库存</a></tr>';
        }
        $str .= "</table>";
       return $str;
    }
    ...
    function GetSpecCheckboxList($type_id, $checked = array()){
        $list = M('Spec')->where("type_id = $type_id and sid = $this->sid")->order('`order` desc')->select();
    ...
    function GetAttrCheckboxList($type_id, $checked = array()){                
        $list = M('GoodsAttribute')->where("type_id = $type_id and attr_index > 0 and sid = $this->sid")->order('`order` desc')->select();
        //$list = M('Spec')->where("1=1")->order('`order` desc')->select();        
        $str = '';
    ...
    function find_parent_cat($cat_id)
    {
        ...
        $cat_list =  M('goods_category')->where(array('sid'=>$this->sid))->getField('id,parent_id,level');
    ...
    function getSortBrands()
    {
        ...
        $brandList =  M("Brands")->where(['sid'=>$this->sid])->select();
        if(!empty($brandList)){
            $newBrands = [];
            foreach($brandList as $key=>$val){
                $firstCharter = getFirstCharter(trim($val['name']));
                $firstCharter = empty($firstCharter) ? 'z' : $firstCharter;
                $newBrands[$firstCharter][] = $val;
            }
            ksort($newBrands);
            S('brandList',$newBrands);
        }
        return $newBrands;
    }
     ...
     *  获取排好序的分类列表     
     */
    function getSortCategory($type = 0)
    {
        //$where = $this->get_shop_all_category($type);
        $where = [];
        $categoryList =  M("GoodsCategory")->cache(true)->where($where)->getField('id,name,parent_id,level');
        $nameList = array();
    ...
    protected function getAllChildCat($shop_list){
        if(!empty($shop_list)){
            $shop_arr = explode(',',$shop_list);
            $all_cat_ids = [];
            foreach($shop_arr as $v){
                $v = intval($v);
                if($v > 0){
                    //获取子分类id
                    $child_ids = getCatGrandson($v);
        ...
        if(!empty($all_cat_ids)){
            $all_cat_ids = implode(',',$all_cat_ids);
        }
        return $all_cat_ids;
    }
}