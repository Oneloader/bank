...
//        }
    ...
    public function role_info(){
		...
		$activity = [];
		$activity['兑换活动'] = M('activity')->field('id,name')->where(['sid'=>$this->sid,'type'=>1,'status'=>1])->select();
		$activity['发放活动'] = M('activity')->field('id,name')->where(['sid'=>$this->sid,'type'=>2,'status'=>1])->select();

        foreach ($activity as $activi=>$value){
            foreach ($value as $acti=>$act){
                if(!empty($detail)){
                    $activity[$activi][$acti]['enable'] = in_array($act['id'], $detail['aid']);
                }
            }
        }
    ...
    public function roleSave(){
    	...
    	$res = $data['data'];
		$res['sid'] = $this->sid;
		$res['aid'] = is_array($data['a_id']) ? implode(',',$data['a_id']) : '';
