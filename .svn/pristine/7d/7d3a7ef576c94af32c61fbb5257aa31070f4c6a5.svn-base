...
if($_SERVER['SERVER_ADDR'] == '127.0.0.1'){
    define('LOCAL_TEST',1); //本地测试
    define('SITE_URL','http://'.$_SERVER['HTTP_HOST']); // 网站域名
}elseif($_SERVER['SERVER_ADDR'] == '114.215.90.145'){
    define('LOCAL_TEST',2); //测试环境
    define('SITE_URL','http://'.$_SERVER['HTTP_HOST']); // 网站域名
}else{
    define('LOCAL_TEST',3); //正式环境
...
define('K11APP_CACHE_TIME',1); // 缓存时间  31104000
