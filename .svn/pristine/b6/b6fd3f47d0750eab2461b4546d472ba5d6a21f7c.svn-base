...
//            $data['new_field'] = json_encode(array_values($data['new_field']));
            if ($data['new_field']){
...
//            successReturn($orders);exit;
    ...
    private function checkCode($code=''){
        ...
        if($code_info['use_status'] == 1){
            return ['status'=>0,'msg'=>'兑换码已经被使用了'];
        }elseif($code_info['use_status'] == 2){
            return ['status'=>0,'msg'=>'兑换码已经失效'];
        }elseif($code_info['use_status'] != 0){
            return ['status'=>0,'msg'=>'兑换码错误'];
        }
