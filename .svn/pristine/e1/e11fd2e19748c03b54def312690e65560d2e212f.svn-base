...
class Think
{
    ...
    private function parseTemplate($template)
    {
        ...
        {
            $default_theme = $this->config['default_theme'] . DS  ;
        ...
        if (0 !== strpos($template, '/')) {
            ...
            if ($controller) {
                if ('' == $template) {
                    // 如果模板文件名为空 按照默认规则定位
                    $template = $default_theme . str_replace('.', DS, $controller) . $depr . $request->action();
                } elseif (false === strpos($template, $depr)) {
                    $template = $default_theme . str_replace('.', DS, $controller) . $depr . $template;
                }else{
                    $template = $default_theme . $template;
                }
            }
        } else {
            $template = $default_theme . str_replace(['/', ':'], $depr, substr($template, 1));
        }
